...
class BisonIntegrationTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    test_dir_ = std::filesystem::temp_directory_path() / "bison_test";
    ...
    bison_path_ = BISON_EXECUTABLE_PATH;
#endif
    // Create a simple grammar file for testing
    grammar_file_ = test_dir_ / "test.y";
    CreateTestGrammar();
  }
  ...
  void CreateTestGrammar() {
    std::ofstream file(grammar_file_);
    file << R"(
%{
#include <stdio.h>
#include <stdlib.h>
...
int main(void) {
 ...
 private:
  ...
  std::filesystem::path grammar_file_;
  std::string bison_path_;

 protected:
  const std::filesystem::path& TestDir() const { return test_dir_; }
  const std::filesystem::path& GrammarFile() const { return grammar_file_; }
  const std::string& BisonPath() const { return bison_path_; }
};
...
TEST_F(BisonIntegrationTest, BisonVersionTest) {
  ...
  std::string version_command = BisonPath() + " --version > /dev/null 2>&1";
  ...
  std::string popen_command = BisonPath() + " --version 2>/dev/null";
  ...
  while (fgets(buffer.data(), static_cast<int>(buffer.size()), pipe) !=
         nullptr) {
    version_output.append(buffer.data());
  }
  pclose(pipe);
...
TEST_F(BisonIntegrationTest, BisonHelpTest) {
  ...
  std::string help_command = BisonPath() + " --help > /dev/null 2>&1";
  int result = std::system(help_command.c_str());
  EXPECT_EQ(result, 0) << "bison --help should work";

  // Capture help output
  std::string popen_help_command = BisonPath() + " --help 2>/dev/null";
  ...
  while (fgets(buffer.data(), static_cast<int>(buffer.size()), pipe) !=
         nullptr) {
    help_output.append(buffer.data());
  }
  pclose(pipe);
...
TEST_F(BisonIntegrationTest, BasicGrammarParsingTest) {
  ASSERT_TRUE(std::filesystem::exists(GrammarFile()))
      << "Test grammar file should exist";

  // Test that bison can parse the grammar without errors
  std::string command = BisonPath() + " --yacc --defines --output=" +
                        (TestDir() / "test.tab.c").string() + " " +
                        GrammarFile().string() + " 2>&1";
  ...
  EXPECT_TRUE(std::filesystem::exists(TestDir() / "test.tab.c"))
      << "Bison should generate C source file";
  EXPECT_TRUE(std::filesystem::exists(TestDir() / "test.tab.h"))
...
TEST_F(BisonIntegrationTest, OutputFormatsTest) {
  ...
  // Test verbose output
  std::string verbose_command = BisonPath() + " --verbose --output=" +
                                (TestDir() / "verbose.tab.c").string() + " " +
                                GrammarFile().string() + " 2>&1";
  ...
  EXPECT_TRUE(std::filesystem::exists(TestDir() / "verbose.output"))
...
TEST_F(BisonIntegrationTest, ErrorHandlingTest) {
  ...
  std::filesystem::path invalid_grammar = TestDir() / "invalid.y";
...
// Missing final %% section
  ...
  std::string command = BisonPath() + " " + invalid_grammar.string() + " 2>&1";
...
TEST_F(BisonIntegrationTest, FeatureSupportTest) {
  ...
  std::string help_command = BisonPath() + " --help 2>/dev/null";
